...
/**
 * ECW DIRTH need record MAX. MID. MIN TC/QD
 */
$tcqdArray = array();
...
class BlockStorageTestEcw extends BlockStorageTest {
  /**
   * ECW Block Size Access Probabilities
  ...
  /**
   * ECW Access Range Distribution Restrictions
   * 50%    First 5%         LBA Group A
   * 30%    Next 15%         LBA Group B
   * 20%    Remaining 80%    LBA Group C
   */
  const ZONE = 'zoned:50/5:30/15:20/80'; 
  const ECW_MAX_ROUND = 25;
  const BLOCK_STORAGE_TEST_ECW_PRECONDITION_INTERVALS = 30;
  const DURATION = 5;
  /**
   * Plot Title
   */
   ...
   */
  protected function BlockStorageTestEcw($options, $rw=NULL) {
    
    if ($rw === NULL) {
        foreach(array('ecw1', 'ecw2', 'ecw3') as $rw) {
          $this->subtests[$rw] = new BlockStorageTestECW($options, $rw);
          $this->subtests[$rw]->test = 'ecw';
  ...
  protected function getReportContent($section, $jobs, $dir) {
    ...
    switch($section){
      case 'access_patten_probabilities':
        if($this->rw == 'ecw1')
          $content = "
          <table style=\"width: 600px; margin-left: auto; margin-right: auto;\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
          <tbody>
          <tr style=\"background-color: #666; color: white;\">
          <td style=\"width: 255px; text-align: left;\">&nbsp; Block Size in Bytes (KiB)</td>
          <td style=\"width: 532px; text-align: center;\">Access Probability Within Each Measurement Period</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 512 bytes (0.5 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">4%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 1024 bytes (1 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 1536 bytes (1.5 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 2048 bytes (2 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 2560 bytes (2.5 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 3072 bytes (3 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 3584 bytes (3.5 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">1%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 4096 bytes (4 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">67%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 8192 bytes (8 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">10%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 16,384 bytes (16 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">7%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 32,768 bytes (32 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">3%</td>
          </tr>
          <tr>
          <td style=\"width: 255px;\">&nbsp; &nbsp; 65,536 bytes (64 KiB)</td>
          <td style=\"width: 532px; text-align: center;\">3%</td>
          </tr>
          <tr style=\"background-color: #666; color: white;\">
          <td style=\"width: 255px;\">&nbsp; Total</td>
          <td style=\"width: 532px; text-align: center;\">100%</td>
          </tr>
          </tbody>
          </table>
          <p>&nbsp;</p>
          <table style=\"width: 596px; margin-left: auto; margin-right: auto;\" border=\"1\" cellspacing=\"1\" cellpadding=\"1\">
          <tbody>
          <tr style=\"background-color: #666; color: white;\">
          <td style=\"width: 211px; text-align: center;\">% of Access within 1 Measurement Period</td>
          <td style=\"width: 234.5px; text-align: center;\">Active Range Restriction</td>
          <td style=\"width: 135.5px; text-align: center;\">Label</td>
          </tr>
          <tr>
          <td style=\"width: 211px;\">&nbsp; 50%</td>
          <td style=\"width: 234.5px;\">&nbsp; First 5%</td>
          <td style=\"width: 135.5px;\">&nbsp; LBA Group A</td>
          </tr>
          <tr>
          <td style=\"width: 211px;\">&nbsp; 30%</td>
          <td style=\"width: 234.5px;\">&nbsp; Next 15%</td>
          <td style=\"width: 135.5px;\">&nbsp; LBA Group B</td>
          </tr>
          <tr>
          <td style=\"width: 211px;\">&nbsp; 20%</td>
          <td style=\"width: 234.5px;\">&nbsp; Remaining 80%</td>
          <td style=\"width: 135.5px;\">&nbsp; LBA Group C</td>
          </tr>
          </tbody>
          </table>";
        break;
      ...
      case 'between_round'://P3
        if($this->rw == 'ecw2'){  
          $label = 'Between Round Pre-Writes, BS=ECWAP';
          foreach(array_keys($this->fio['wdpc']) as $i) {
            $job = isset($this->fio['wdpc'][$i]['jobs'][0]['jobname']) ? $this->fio['wdpc'][$i]['jobs'][0]['jobname'] : NULL;
      ...
      case 'dv_iops'://P4
        if($this->rw == 'ecw2'){
          ...
          $settings['xMin'] = $this->subtests['ecw2']->wdpcComplete - 5;
          $settings['yMin'] = 0;
      ...
      case 'demand_variation'://P6
        if($this->rw == 'ecw2'){  
          $str = sprintf("/^x%d\-40_60\-rand\-TC([0-9]+)\-QD([0-9]+)/", $this->subtests['ecw2']->wdpcComplete);
          foreach(array_keys($jobs) as $job){
            $label = NULL;
            $xTic = NULL;
            if(preg_match($str, $job, $m) && isset($jobs[$job]['write']['iops'])){
          ...
          $title = 'P6 ECWT Demand Variation';
          $content = sprintf(self::plotTitle, $title);
      ...
      case 'demand_intensity'://P7
        if($this->rw == 'ecw2'){  
          ...
          $title = 'P7 ECWT Demand Intensity';
          $content = sprintf(self::plotTitle, $title);
      ...
      case 'min_iops_histogram'://P14
        if($this->rw == 'ecw3'){  
          ...
          global $tcqdArray;
          if($type == 'max'){
            if(isset($tcqdArray['ECW']['MAX'])){
              $data = explode('_', $tcqdArray['ECW']['MAX']);
              $num = $data[1];
            }
          }elseif($type == 'mid'){
            if(isset($tcqdArray['ECW']['MID'])){
              $data = explode('_', $tcqdArray['ECW']['MID']);
              $num = $data[1];
            }
          }else{
            if(isset($tcqdArray['ECW']['MIN'])){
              $data = explode('_', $tcqdArray['ECW']['MIN']);
              $num = $data[1];
            }
          }
          ...
          for($n=1; $n<=10; $n++){  //10 mins
            for($x=1; $x<=$num; $x++){  //QD number will decide how many logs
              $fileName = sprintf("%s/ecw-fio-lat-%s-%d_lat.%d.log", $fdir, $type, $n, $x);
          ...
          //delete log files
          $fileName = sprintf("%s/ecw-fio-lat-%s*.log", $fdir, $type);                
          exec("rm -f $fileName");
  ...
  protected function getReportSections(){
    ...
    return array(
      'access_patten_probabilities' => 'Access Patten Block Sizes & Access Probabilities - 40:60 RW',
  ...
  protected function getSetupParameters(){
    if (isset($this->controller)) return $this->controller->getSetupParameters();
      ...
      return array(        
        ...
        '&nbsp;&nbsp;SS Rounds' => $this->subtests['ecw1']->wdpc !== NULL ? sprintf('%d - %d', $this->subtests['ecw1']->wdpcComplete - 4, $this->subtests['ecw1']->wdpcComplete) : 'N/A',
        'Inter-Round Pre W' => 'ECW',
        '&nbsp;&nbsp;R/W % ' => '0/100',
        '&nbsp;&nbsp;TOIO - TC/QD ' => 'TC 32 / QD 32',
  ...
  protected function getSubtitle($section){    
    return 'ECW Block Size / Probability Workload';
  }
  ...
  public function wdpc() {
    ...
    if ($this->rw !== NULL) {
      ...
      if ($rw == 'ecw1') {  
        ...
        print_msg(sprintf('Initiating workload dependent preconditioning and steady state for ECW test'), $this->verbose, __FILE__, __LINE__);
        
        $ssMetrics = array();
    
        for($x=1; $x<=$max; $x++) {
    
          for($n=1; $n<=self::BLOCK_STORAGE_TEST_ECW_PRECONDITION_INTERVALS; $n++) {
            $name = sprintf('x%d-0_100-rand-n%02d', $x, $n);
            print_msg(sprintf('Starting %d sec ECW rand write preconditioning round %d of %d, test %d of %d [name=%s]', $this->options['wd_test_duration'], $x, $max, $n, self::BLOCK_STORAGE_TEST_ECW_PRECONDITION_INTERVALS, $name), $this->verbose, __FILE__, __LINE__);
            ...
            if ($results && $n==1) {
              ...
              print_msg(sprintf('Added IOPS metric %d from preconditioning round %d of %d for ECW steady state verification', $iops, $x, $max), $this->verbose, __FILE__, __LINE__);
              $ssMetrics[$x] = $iops;
              ...
              if ($x >= 5) {
                ...
                print_msg(sprintf('ECW preconditioning test %d of %d complete and >= 5 rounds finished - checking if steady state has been achieved using ECWAP write IOPS metrics [%s],[%s]', $x, $max, implode(',', array_keys($metrics)), implode(',', $metrics)), $this->verbose, __FILE__, __LINE__);
                
                if ($this->isSteadyState($metrics, $x)) {
                  print_msg(sprintf('ECW steady state achieved - testing will stop'), $this->verbose, __FILE__, __LINE__);
                  $status = TRUE;
                  break;
                } else{ 
                  print_msg(sprintf('ECW steady state NOT achieved'), $this->verbose, __FILE__, __LINE__);
                }
        ...
        $this->wdpcComplete = $x;
        $this->wdpcIntervals = self::BLOCK_STORAGE_TEST_ECW_PRECONDITION_INTERVALS;
        $this->wdpc = $status;
        ...
        print_msg(sprintf('ECW preconditioning complete - beginning wait state test segments'), $this->verbose, __FILE__, __LINE__);
        ...
        for($x=1; $x<=$max; $x++){
          ...
          print_msg(sprintf('Starting %d sec ECW rand write preconditioning round %d of %d [name=%s]', $preWriteTime, $x, $max, $name), $this->verbose, __FILE__, __LINE__);
          ...
          foreach($tcArray as $tc){            
            ...
            foreach($qdArray as $qd){
              ...
              print_msg(sprintf('Starting %d sec ECW rand write round %d of %d, test %d of %d [name=%s]', $this->options['wd_test_duration'], $x, $max, $n, $composite, $name), $this->verbose, __FILE__, __LINE__);
              ...
              if ($results && $tc==32 && $qd==32) {
                $iops = $results['jobs'][0]['write']['iops'];
                print_msg(sprintf('Added IOPS metric %d from round %d of %d for ECW steady state verification', $iops, $x, $max), $this->verbose, __FILE__, __LINE__);
                $ssMetrics[$x] = $iops;
              }
          ...
          if ($x >= 5) {
            $metrics = array();
            for($i=4; $i>=0; $i--) $metrics[$x-$i] = $ssMetrics[$x-$i];
            print_msg(sprintf('ECW test %d of %d complete and >= 5 rounds finished - checking if steady state has been achieved using IOPS metrics [%s],[%s]', $x, $max, implode(',', array_keys($metrics)), implode(',', $metrics)), $this->verbose, __FILE__, __LINE__);
            ...
            if ($this->isSteadyState($metrics, $x)) {
              ...
              global $tcqdArray;
              $tcqdArray['ECW']['MAX'] = isset($maxTcqd)? $maxTcqd : NULL;
              $tcqdArray['ECW']['MID'] = isset($midTcqd)? $midTcqd : NULL;
              $tcqdArray['ECW']['MIN'] = '1_1';
        ...
        $this->wdpcComplete = $x;
        $this->subtests['ecw2']->wdpcComplete = $x;
        ...
        $this->wdpc = $status;
          ...
          foreach($tcqdArray['ECW'] as $type => $value){
            ...
            foreach($tests as $rw){
              ...
              for($n=1; $n<=$max; $n++){
                ...
                print_msg(sprintf('Starting %d sec ECW rand write , test %d of %d [name=%s]', $this->options['wd_test_duration'], $n, $max, $name), $this->verbose, __FILE__, __LINE__);
                if($rw == '40_60') {
                  $params['write_lat_log'] = sprintf("ecw-fio-lat-%s-%d",strtolower($type), $n);
                }
                $params['name'] = $name;
                if ($fio = $this->fio($params, 'wdpc')) {
                  print_msg(sprintf('Test %s was successful', $name), $this->verbose, __FILE__, __LINE__);
            ...
            $x = ($x == count($tcqdArray['ECW']))? $x : ++$x;            
          }//end foreach($tcqdArray['ECW'] as $type => $value)
    ...
    // main test controller
      ...
      foreach(array_keys($this->subtests) as $i => $rw) {
        print_msg(sprintf('Starting workload dependent preconditioning for ECW R/W %s (%d of %d)', $rw, $i+1, count($this->subtests)), $this->verbose, __FILE__, __LINE__);
