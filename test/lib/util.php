...
function get_sys_info() {
  ...
  try{
    ...
    if($data = $stmt->fetch(PDO::FETCH_ASSOC)){
      ...
      $sys_info['mfgr'] = $info->mfgr;
      $sys_info['modelno']= $info->modelno;
      $sys_info['sn']= $info->sn;
      $sys_info['fwv']= $info->fwv;
      $sys_info['interface']= $info->interface;
      $sys_info['nandtype']= $info->nandtype;
      $sys_info['test_sn'] = $info->testsn;   
      $dbLink = null;
    }
  ...
  $tmpFile = $para['report_folder'].'tmp';
...
function ConnectDB(){
  ...
  $userPassword = "pts";
  try{
      $dbLink = new PDO("mysql:host={$host};dbname={$dbName};charset=utf8", $userName, $userPassword);
      return $dbLink;
  }
  catch(PDOException $e){
    print_msg(sprintf('Fail to connect to DB, information：%s', $e->getMessage()), $this->verbose, __FILE__, __LINE__, TRUE);
  }
}
?>